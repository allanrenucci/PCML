...
Tr.concat = train.X_cnn; %[normalize2(train.X_hog) * reduceDimension(normalize2(train.X_hog))];

Tr.idxs = 1:2:size(Tr.concat,1);
Tr.X = Tr.concat(Tr.idxs,:);
Tr.y = train.y(Tr.idxs);

Te.idxs = 2:2:size(Tr.concat,1);
Te.X = Tr.concat(Te.idxs,:);
...
Tl = reduceDimension(Tr.normX);
Tr.normX = Tr.normX * Tl;

Te.normX = normalize(Te.X, mu, sigma);  % normalize test data
Te.normX = Te.normX * Tl;
...
nn = nnsetup([size(Tr.normX,2) 10 4]);
nn.learningRate = 2;                        
...
for i=20:30  % just 10 of them, though there are thousands
    ...
    title(sprintf('Label: %s, Pred: %s', labelToString(train.y(Te.idxs(i))), labelToString(classVote(i))));
